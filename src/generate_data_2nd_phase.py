...
# --- SỬA ĐOẠN NÀY ---
# Định nghĩa lại color_map chỉ với 4 lớp để khớp với model 4 classes của bạn
color_map = [
    [255, 255, 255],    # 0: Background
    [0, 128, 0],        # 1: Viable tumor
    [255, 0, 0],        # 2: Necrosis
    [0, 0, 255]         # 3: Non-tumor tissue (hoặc class thứ 4 của bạn)
]
# --------------------
...
def infer(image_filepath, label_filepath, size=256):
    ...
    print(f"\n[DEBUG] Đang xử lý: {os.path.basename(image_filepath)}")
    ...
    # Tính thời gian đọc
    t1 = cv2.getTickCount()
    print(f"[DEBUG] Đã đọc ảnh xong. Kích thước: {img.shape}. Thời gian: {(t1-t0)/cv2.getTickFrequency():.2f}s")
    ...
    print(f"[DEBUG] Số lượng patch cần chạy: {len(image_patches)} patch(es)")
    ...
    # 3. Chạy vòng lặp dự đoán (Thêm tqdm để thấy nó chạy)
    print("[DEBUG] Bắt đầu dự đoán từng patch...")
    for i, (im_patch, gt_patch) in enumerate(tqdm(zip(image_patches, label_patches), total=len(image_patches), desc="Infer patches")):
        
        # ... Xử lý background như cũ ...
    ...
    print("[DEBUG] Đang gộp ảnh (Combine)...")
    # Lưu ý: color_map phải có độ dài bằng số class (4)
